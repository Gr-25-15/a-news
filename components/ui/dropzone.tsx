...
const useDropzone = <TUploadRes, TUploadError = string>(
  ...
  const onRetry = useCallback(
    async (id: string) => {
      ...
      dispatch({ type: "update-status", id, status: "pending", tries: fileStatus.tries + 1 });
      await _uploadFile(fileStatus.file, id);
    },
    [canRetry, fileStatuses, dispatch, _uploadFile],
  );
...
interface DropzoneRemoveFileProps extends React.ComponentProps<"button"> {}
...
interface DropzoneRetryFileProps extends React.ComponentProps<"button"> {}
